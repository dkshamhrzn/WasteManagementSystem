...
// Interface to define structure of pickup status data
...
const RequestPickupScreen = () => {
  ...
  // Retrieves email from SecureStore or AsyncStorage
  ...
  // Fetch pickup request status history for the logged-in user
  const fetchPickupStatus = async (userEmail: string) => {
    ...
    try {
      ...
      // Sort requests by most recent date and time
  ...
  const validateForm = () => {
    ...
    if (!quantity || isNaN(Number(quantity)))
      newErrors.quantity = "Enter a valid quantity (e.g., 5.3).";
